...
def whatsapp_reply():
    try:
        ...
        if user_state.get(sender_number) == 'admin_menu':
            if incoming_msg == '1':
                msg.body("مرحباً بك في إدارة المركز، المسؤول: د. سعود الواصلي")
            elif incoming_msg == '2':
                msg.body("مرحباً بك في إدارة الابتكار، المسؤول: د. ليلى أحمد باهمام")
            elif incoming_msg == '3':
                msg.body("مرحباً بك في إدارة نقل التقنية، المسؤول: شاغر حالياً")
            elif incoming_msg == '4':
                msg.body("مرحباً بك في إدارة تتجير المعرفة، المسؤول: د. رائد إسماعيل فلمبان")
            elif incoming_msg == '5':
                msg.body("مرحباً بك في إدارة ريادة الأعمال، المسؤول: د. حسن علي السقاف")
            else:
                msg.body("❓ يرجى اختيار رقم من القائمة.")
            return str(response)
